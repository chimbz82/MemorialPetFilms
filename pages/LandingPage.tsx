  ...
  return (
    <div className="min-h-screen bg-brand-main">
      ...
      >
        ...
        <div className="absolute top-1/4 left-0 w-96 h-96 bg-brand-primary/10 rounded-full blur-[100px] -translate-x-1/2" />
        <div className="absolute bottom-1/4 right-0 w-96 h-96 bg-brand-secondary/10 rounded-full blur-[100px] translate-x-1/2" />
      </section>
      ...
      <section id="how-it-works" ref={howItWorksRef} className="py-32 px-6 bg-brand-section">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-24">
            ...
            <p className="text-brand-body/70 max-w-xl mx-auto leading-relaxed">
          ...
          <div className="grid md:grid-cols-3 gap-16">
            {[
              ...
              <div key={idx} className="relative text-center md:text-left">
                <div className="text-brand-emotion/40 text-[120px] font-serif absolute -top-16 -left-4 leading-none select-none">
                  0{idx + 1}
                </div>
                <div className="relative z-10 pt-4">
                  ...
                  <p className="text-brand-body leading-relaxed text-sm">{item.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="templates" ref={templatesRef} className="py-32 px-6 bg-brand-main">
        <div className="max-w-7xl mx-auto">
            ...
            {TEMPLATES.slice(0, 3).map((template) => (
              ...
              >
                ...
                <div className="p-10">
                  ...
                  <p className="text-sm text-brand-body/70 leading-relaxed font-light">{template.description}</p>
                </div>
              </button>
            ))}
          </div>
          ...
          <div className="text-center mt-16">
            <Button variant="outline" onClick={() => setShowAllTemplates(true)} className="px-10">
              View all 9 styles
            </Button>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {Object.entries(PACKAGES).map(([key, pkg]) => (
              <div
                key={key}
                className={`border border-brand-section rounded-[3rem] p-12 flex flex-col transition-all duration-500 ${
                  key === 'premium'
                    ? 'bg-white border-brand-primary shadow-2xl scale-105 z-10'
                    : 'bg-brand-section/50 hover:border-brand-primary'
                }`}
              >
                {key === 'premium' && (
                  <span className="bg-brand-secondary text-white text-[10px] font-bold uppercase tracking-widest py-1.5 px-5 rounded-full self-start mb-8 shadow-sm">
                    Most Chosen
                  </span>
                )}
                ...
                <Button variant={key === 'premium' ? 'primary' : 'outline'} className="w-full" onClick={onStart}>
                  Select {key}
                </Button>
              </div>
            ))}
          ...
          <div className="max-w-6xl mx-auto">
            <div className="flex justify-between items-center mb-16">
              ...
              <button
                ...
                className="text-brand-heading/50 hover:text-brand-heading transition-colors uppercase tracking-widest text-xs font-bold"
              >
                Close ×
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12">
              {TEMPLATES.map((template) => (
                <button key={template.id} className="group text-left focus:outline-none" onClick={() => setSelectedPreview(template)}>
                  ...
                  >
                    <span className="text-[10px] font-bold text-brand-body/20 uppercase tracking-widest group-hover:scale-110 transition-transform">
            ...
            <div className="mt-20 text-center pb-20">
              <Button onClick={onStart} className="px-12">
                Start Your Tribute
              </Button>
            </div>
          </div>
      ...
      {/* Preview Modal */}
      {selectedPreview && (
        <TemplateModal template={selectedPreview} onClose={() => setSelectedPreview(null)} onSelect={() => onStart()} />
      )}
    </div>
  );
};
